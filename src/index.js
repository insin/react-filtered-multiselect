...
function getItemsByProp(arr, prop, values) {
  ...
  for (
    let i = 0, la = arr.length, lv = values.length;
    i < la && found < lv;
    i++
  ) {
...
class FilteredMultiSelect extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    if (
      ...
      prevProps.selectedOptions.length !== this.props.selectedOptions.length
    ) {
      this.setState(
        {
  ...
  _getClassName(name, ...modifiers) {
    ...
    for (let i = 0, l = modifiers.length; i < l; i++) {
      if (modifiers[i]) {
        classNames.push(
          this.props.classNames[modifiers[i]] ||
            DEFAULT_CLASS_NAMES[modifiers[i]]
        );
      }
    }
  ...
  _onFilterKeyPress = (e) => {
    if (e.key === "Enter") {
      ...
      if (this.state.filteredOptions.length === 1) {
        ...
        let selectedOptions = this.props.selectedOptions.concat([
          selectedOption,
        ]);
      ...
      <div className={className}>
        ...
        <button
          ...
          className={this._getClassName(
            "button",
            hasSelectedOptions && "buttonActive"
          )}
